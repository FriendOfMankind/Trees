...
### What survives with no signal, and what doesn't

Three separate things, and they fail independently:

| Layer | Offline | How |
|---|---|---|
| Leaflet | always | vendored in `vendor/leaflet/` |
| Pins and lines | always | static data in `data.js`, nothing fetched |
| Tiles | **only if prefetched** | `sw.js` caches them; the Map tab's tile panel downloads a region on request |

So the lines are the reliable layer and the tiles are the conditional one —
the reverse of what you'd assume. A route drawn from baked geometry is there
with the server unreachable; the basemap under it is only there if someone
pressed the download button while they had signal.

That is also why route geometry feeds the prefetch bounds and not just the
framing: a leg that swings outside the box its two endpoints make would
otherwise have its tiles left uncached, and you would lose the basemap exactly
where the road goes somewhere non-obvious.

Committing pre-rendered tiles to the repo is not the answer and shouldn't be
attempted: OSM's tile policy treats bulk downloading as abuse, and it would add
tens of megabytes per trip. The prefetch is user-driven and capped for the same
reason — see the note at the top of `sw.js`.
