...
const GEAR = [
  ...
  },
  {
    ...
    note: "One burner, one pot, one pan. The meal plans are built to that exact constraint. ⚠️ <b>CORRECTED Sept 14 2026 — most of this category was marked <code>own</code> and is not owned.</b> Colin said plainly that the pot, the pan, the lid, the thermos, the cooler and the knife all still have to be bought. Every one of them had been sitting here as <code>own</code> since the file was written, inherited from the Maui packing list rather than from anyone checking a cupboard, and two trips' meal plans were built on top of that assumption. <b>Nothing in this category goes back to <code>own</code> without Colin saying so about that specific item.</b>",
    items: [
      { name: "One burner", state: "own", note: "✅ <b>Confirmed by Colin, Sept 16 2026.</b> Briefly marked unknown when the rest of this category turned out not to be owned — it is the single point of failure for both 2026 meal plans, since 13 of Kentucky's 18 slots need a flame, so it was not going to sit here as an assumption. Confirm it takes screw-on isobutane before buying canisters." },
      { name: "2L pot with lid", state: "need", note: "<b>2 litres, and the lid is not optional</b> — it drains, steams and holds heat, and the boil-bag primitive that makes a waterless campsite survivable is a pot that only ever held water." },
      { name: "8–10 in pan", state: "need", note: "Eggs, quesadillas, the foil burrito and the ribeye. One burner means everything is sequential, so one pan is the constraint the recipes are written to." },
      { name: "Insulated mug, spork", state: "unknown", note: "Bundled into the old \"burner, pot, pan, mug, spork\" row that turned out not to be true. Not separately confirmed." },
      { name: "Folding knife", state: "need", note: "On both trips' kit lists, never in this locker. Salami, peppers, potatoes, the sun-dried tomatoes for the thermos dinner." },
      { name: "Wide-mouth thermos", state: "need", note: "⚠️ <b>Was marked owned; it is not.</b> <b>Wide-mouth, 500 ml or larger</b> — narrow coffee flasks cannot be loaded with couscous, chickpeas and shredded chicken, and cannot be eaten out of. Load-bearing: the standing routine is snack → hike → hot thermos meal at the turnaround → hike out, and Kentucky has two meals that exist only because of it (Wednesday's oats on the ridge, Thursday's dinner at the overlook). <b>Since one is being bought anyway, buy two</b> — the Sept 2026 \"one, and staying at one\" decision was made about a flask already in a cupboard, and the reason to hold the line on it disappears when both are a purchase. A second unlocks the pre-dawn hot chocolate that the meal currently displaces." },
      { name: "48 qt cooler", state: "need", note: "⚠️ <b>Was marked owned; it is not.</b> <b>Buy 48 qt, not smaller.</b> The entire cooler doctrine is sized to it: the three-zone model, ~2.5 days unaided at 75°F, and Kentucky's five nights on one cooler with a single resupply. A 25–35 qt box does not hold Zone 1 plus a rotisserie chicken plus a block of ice. Frozen meals in flat quart bags <i>are</i> the ice — buy a <b>block</b> at resupply, not cubes.",
