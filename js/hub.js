...
(function () {
  ...
  /* A trip page links one recipe as "?meal=<id>#kitchen". The hash belongs to
     js/ui.js and has to stay an exact tab name, so the recipe rides in the
     query string — the tab still activates the ordinary way. */
  function focusMealFromUrl() {
    let id;
    try { id = new URLSearchParams(location.search).get("meal"); } catch (e) { return; }
    if (!id) return;
    focusMeal(id);
  }
  ...
  function focusMeal(id) {
    ...
    /* Collapse the reference material so the named recipe is the first thing
       on the panel. Scrolling to it instead would mean animating the page on
       load and hoping the layout has settled; hiding is deterministic. */
    setKitchenFocused(true);

    $("[data-kfilter]").addEventListener("click", () => {
      kitchenFilter = "all";
      setKitchenFocused(false);
      renderKitchenFilters();
      renderMealGrid();
    });
  }

  function setKitchenFocused(on) {
    for (const id of ["#kitchen-intro", "#kitchen-extras"]) {
      const el = $(id);
      if (el) el.hidden = on;
    }
  }
  ...
  function init() {
    ...
    initHubMap();
    /* After initTabs: the kitchen panel has to be visible before anything
       tries to scroll to a card inside it, or it scrolls to a hidden box. */
    focusMealFromUrl();
