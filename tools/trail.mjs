...
const END = "  // <<< TRAILS";
const block =
  `${START} — generated by tools/trail.mjs. Re-run the tool rather than
  // hand-editing; the next --write overwrites everything between these markers.
  trails: [
${routes.map((r) =>
  `    {\n` +
...
if (i === -1 || j === -1) {
  console.log(`${rel} has no generated-trails block. Paste this into window.TRIP_DATA, keeping the marker lines:\n`);
  console.log(block);
  process.exit(0);
}
const { writeFileSync } = await import("node:fs");
writeFileSync(join(ROOT, rel), source.slice(0, i) + block + source.slice(j + END.length));
console.log(`Wrote ${routes.length} trail(s) into ${rel}. Now run: node tools/validate.mjs`);
